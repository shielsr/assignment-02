...
function liveCounting() {
    ...
    if (valuesTotal() > 12) {
        ...
        document.getElementById("live-count").classList.add('error-message');
    } else if (valuesTotal() === 12) {
        document.getElementById("live-count").classList.remove('error-message');
...
function generateRoll(event) {
    ...
    const errorMessage = document.getElementById("roll-error-message");
...
function errorCheckOnSubmit(event) {
    ...
    document.querySelectorAll('.error-message-order-form').forEach(span => span.textContent = '');
